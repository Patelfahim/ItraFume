...
exports.getDashboardStats = catchAsync(async (req, res) => {
  const [totalOrders, totalRevenueAgg, pendingOrders, totalUsers] = await Promise.all([
  ...
  const recentOrders = await Order.find({ isPaid: true }).populate('user', 'name').sort({ createdAt: -1 }).limit(5);
