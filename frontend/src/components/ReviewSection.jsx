...
import { FiThumbsUp, FiCheckCircle, FiPlay } from 'react-icons/fi';
import StarRating from './StarRating';
import MediaUploader from './MediaUploader';
import api from '../api/axios';
import { useAuth } from '../context/AuthContext';

const ReviewSection = ({ productId, reviews, onReviewAdded, onHelpfulToggled }) => {
  const { user } = useAuth();
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-display text-2xl">Customer Reviews ({reviews?.length || 0})</h2>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-surface-container-low rounded-md p-5 mb-8 space-y-4">
          <div>
          ...
          <div>
            <label className="block text-sm font-semibold mb-1">Add Photos or Videos (optional)</label>
            <MediaUploader files={files} onChange={setFiles} maxFiles={5} label="Share photos or videos of the product —" />
          </div>
      ...
      {lightbox && (
        ...
        >
          {lightbox.type === 'video' ? (
            ...
            <img src={lightbox.url} alt="Review media" className="max-h-[85vh] max-w-full rounded" />
          )}
        </div>
      )}
      ...
      <div className="space-y-6">
        {reviews?.length === 0 && (
          <p className="text-on-surface-variant text-sm">No reviews yet. Be the first to share your experience!</p>
        )}
        {reviews?.map((review) => (
          <div key={review._id} className="border-b border-surface-container-high pb-6">
                ...
                {review.media.map((m, idx) => (
                  ...
                  >
                    {m.type === 'video' ? (
                      ...
                      <img src={m.url} alt="Review upload" className="w-full h-full object-cover" />
                    )}
                  </button>
                ))}
              </div>
            ...
            {review.adminReply?.message && (
              <div className="mt-3 bg-surface-container-low rounded-sm p-3 text-sm">
                <span className="font-semibold text-primary">ItraFume Team: </span>
                {review.adminReply.message}
              </div>
            )}
