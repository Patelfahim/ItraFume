...
import { FiPlay, FiMinus, FiPlus, FiShoppingBag, FiTruck, FiShield } from 'react-icons/fi';
  ...
  return (
    <div className="container-max px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="text-xs text-on-surface-variant mb-6 flex gap-2">
        <Link to="/">Home</Link> / <Link to="/shop">Shop</Link> / <span className="text-on-surface">{product.name}</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">
        ...
        <div>
          <div className="aspect-square bg-surface-container rounded-md overflow-hidden mb-4">
            {currentMedia?.type === 'video' ? (
              <video src={currentMedia.url} controls autoPlay muted loop className="w-full h-full object-cover" />
            ) : (
        ...
        {/* Product info */}
        <div>
          <p className="text-xs uppercase tracking-wider text-secondary mb-2">{product.category} &middot; {product.concentration}</p>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm text-on-surface-variant">
              {product.ratingsAverage?.toFixed(1) || '0.0'} ({product.ratingsCount || 0} reviews)
            </span>
          </div>

          <p className="text-on-surface-variant leading-relaxed mb-6">{product.shortDescription}</p>

          <div className="flex items-baseline gap-3 mb-6">
            <span className="font-display text-3xl text-on-surface">₹{selectedVariant?.price?.toFixed(0)}</span>
            {hasDiscount && (
              <span className="text-lg text-on-surface-variant line-through">₹{selectedVariant.compareAtPrice.toFixed(0)}</span>
            )}
            {hasDiscount && (
          ...
          <div className="mb-6">
            ...
            {selectedVariant?.stock > 0 && selectedVariant.stock <= 5 && (
              <p className="text-xs text-error mt-2">Only {selectedVariant.stock} left in stock!</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center border border-outline-variant rounded-sm">
              <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="p-3" aria-label="Decrease quantity">
                <FiMinus size={14} />
              </button>
            ...
            >
              <FiShoppingBag /> {selectedVariant?.stock === 0 ? 'Out of Stock' : 'Add to Cart'}
            </button>
          </div>

          <div className="grid grid-cols-2 gap-4 mb-8 text-xs text-on-surface-variant">
            <div className="flex items-center gap-2"><FiTruck /> Free shipping over ₹999</div>
            <div className="flex items-center gap-2"><FiShield /> 100% authentic, secure checkout</div>
          </div>

          {/* Fragrance notes */}
          {(product.fragranceNotes?.top?.length > 0 || product.fragranceNotes?.middle?.length > 0) && (
            <div className="border-t border-surface-container-high pt-6 mb-6">
              ...
              <div className="space-y-2 text-sm">
                {product.fragranceNotes.top?.length > 0 && (
                  <p><span className="font-semibold">Top:</span> {product.fragranceNotes.top.join(', ')}</p>
                )}
                {product.fragranceNotes.middle?.length > 0 && (
                  <p><span className="font-semibold">Middle:</span> {product.fragranceNotes.middle.join(', ')}</p>
                )}
                {product.fragranceNotes.base?.length > 0 && (
                  <p><span className="font-semibold">Base:</span> {product.fragranceNotes.base.join(', ')}</p>
                )}
              </div>
            </div>
          )}
      ...
      <div className="mt-16 pt-10 border-t border-surface-container-high">
        <ReviewSection
          ...
          onHelpfulToggled={(reviewId, helpfulVotes) => {
            setReviews((prev) => prev.map((r) => (r._id === reviewId ? { ...r, helpfulVotes } : r)));
          }}
        />
      </div>
    </div>
